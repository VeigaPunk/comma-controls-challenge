"""Stable, collision-free segment identifier built from the first 80 timesteps
of (target_lataccel, roll_lataccel, v_ego, a_ego). Used by both the offline
builder and the runtime controller -- they must produce identical fingerprints.
"""
import hashlib
...
def fingerprint_from_observations(
    obs: Iterable[Tuple[float, float, float, float]],
) -> str:
